...
- [x] T001 Confirm the post-002 baseline: `python scripts/build_blog.py --out _site && python
  scripts/verify_build.py --out _site` → **163 checks, 0 failures** (the bar to grow).

## Phase 2: Foundational (blocking prerequisites)

- [x] T002 Constitution finalized at **v1.3.0** in `.specify/memory/constitution.md` (Principle VIII
...
- [x] T003 [P] Read & confirm the portfolio identity source of truth: `index.html` `Person` JSON-LD
...
- [x] T004 [US1] Add `AUTHOR_KNOWS_ABOUT` to `scripts/blog/config.py` — the portfolio
...
- [x] T005 [US1] Emit `knowsAbout` in `scripts/blog/seo.py` `_author_node(..., full=True)` (so it lands
  on `BlogPosting.author` for posts and `Blog.author` on the index); leave the lean `full=False` node
  unchanged. (depends on T004)
- [x] T006 [US1] `scripts/verify_build.py`: parse the portfolio `Person` from `index.html` once
...
- [x] T007 [US1] `scripts/verify_build.py`: per post + index, assert V-ID6 `jobTitle`, V-ID7
...
- [x] T008 [US2] `scripts/verify_build.py`: assert V-CAT1 every declared category (incl. empty Compose/
...
- [x] T009 [P] [US3] Add a deterministic heading-slug + per-post uniqueness allocator to
...
- [x] T010 [P] [US3] Add `id="{{ID}}"` to the heading element in
  `templates/blog/partials/block-h2.html` — no other markup/style change (invisible). (pairs with T009)
- [x] T011 [US3] `scripts/verify_build.py`: assert V-HA1 every body heading has a non-empty `id`,
...
- [x] T012 [US4] In `index.html`, wrap the `@font-face` `<style>` block in
  `<!--PORTFOLIO-FONTS:START-->` / `<!--PORTFOLIO-FONTS:END-->` markers (no byte change to the faces
  yet). (depends on T002)
- [x] T013 [US4] Snapshot `index.html` → `assets/portfolio-fonts/index.baseline.html` (recoverable
  original; ensure the path is NOT served — not in `ROOT_COPY_ALLOWLIST`, not a copied asset tree).
  (depends on T012)
- [x] T014 [US4] `scripts/verify_build.py`: implement the font-fidelity proof (V-FZ1–6) against the
  ...
  verify_build.py)
- [x] T015 [US4] Subset: delete the 32 non-Latin `@font-face` rules (Cyrillic, Cyrillic-ext, Greek,
...
- [x] T016 [US5] `scripts/verify_build.py`: assert V-LC1 `<meta name="author">==AUTHOR_NAME`, V-LC2
  `inLanguage==LOCALE` (posts + index `Blog`), V-LC3 `og:locale==OG_LOCALE` on every page. (sequential
  in verify_build.py)
- [x] T017 [US5] `scripts/verify_build.py`: assert V-TAG1 keywords-contains-every-tag, V-TAG2 one
  `article:tag` per tag, V-TAG3 `BlogPosting.keywords` contains every tag — for each tagged post.
  (sequential in verify_build.py)
- [x] T018 [P] [US5] Set `specs/001-obsidian-blog/spec.md` Status `Draft → Implemented` and
  `specs/002-seo-geo-publish/spec.md` Status `Ready for Planning → Implemented` (add a one-line shipped
  note). (independent file)
- [x] T019 [P] [US5] Set this spec's Status to reflect progress (`Draft → Implemented` at the end).
  (independent file)
...
- [x] T020 Full build + verify: `python scripts/build_blog.py --out _site && python
  scripts/verify_build.py --out _site` → **0 failures, checks > 163**. Iterate to green.
- [x] T021 Determinism check: build twice to two dirs and diff (`_site` vs `_site2`) → byte-identical
  (heading ids stable, no `today()`).
- [x] T022 Portfolio integrity: confirm `_site/index.html` byte-identical to repo `index.html` outside
...
- [x] T023 Update `PROJECT_CONTEXT.md` (positioning/identity, anchors, font outcome landed/deferred,
  expanded verifier + new count, constitution v1.3.0, 003 folder, resolved inherited gaps).
- [x] T024 Update `CLAUDE.md` active-feature pointer to 003 (managed agent-context).
