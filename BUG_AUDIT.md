...
- [ ] **BUG-003 — False-positive GFM table: a text line containing `|` followed by a dash-run line is parsed as a table.**
...
- [ ] **BUG-004 — URL-scheme allow-list can be bypassed by an embedded NUL/control char (`java\x00script:`).**
...
- [ ] **BUG-011 — Author content containing a literal `{{TOKEN}}` triggers a false "unresolved template tokens" build failure.**
...
- [ ] **BUG-012 — A paragraph immediately after a table that contains a `|` is absorbed as a table row.**
